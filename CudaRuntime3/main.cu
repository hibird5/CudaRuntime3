...
//	cudaMemcpy(pop_vals, agent_pos, 30 * sizeof(float), ::cudaMemcpyDeviceToHost);
//
//
...
//	for (int j = 0; j < 10; ++j)
//	{
//		cout <<  pop_vals[j * NUM_OF_DIMS] << " " << pop_vals[j * NUM_OF_DIMS + 1] << " " << pop_vals[j * NUM_OF_DIMS + 2] << endl;
//	}
//	calc_distances<<<agents, DIMS_TO_LOG_HALF>>>(agent_pos, tmp_distance, dist_gwo);
//
//	cudaMemcpy(pop_back, dist_gwo, 10 * sizeof(float), ::cudaMemcpyDeviceToHost);
//
//	for (int j = 0; j < 10; ++j)
//				{
//					cout<< pop_back[j] << ", " << endl;
//				}
//
