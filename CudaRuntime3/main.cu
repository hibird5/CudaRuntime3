...
__global__ void tet(int* gg)
{
	gg[0] = gridDim.x; gg[1] = gridDim.y;
	gg[2] = blockDim.x; gg[3] = blockDim.y;
}
...
int main()
{
	...
	cost_func << <1, num_of_agents>> > (agent_pos, agent_val);
	...
	//	ffa << <agents, num_of_dims >> > (5, 1, 0.001, a, b, r_a, agent_pos, ff_new_poss, agent_val);
	//	cudaDeviceSynchronize();
	//	cost_func << <num_of_agents, num_of_agents>> > (ff_new_poss, ff_new_vals);
	//
	//	cudaDeviceSynchronize();
	...
	//	searchForBestKernel << <1, num_of_agents_half >> > (agent_val, indice);
	//
	//	err1 = cudaMemcpy(ind, indice, num_of_agents * sizeof(unsigned int), ::cudaMemcpyDeviceToHost);
	//	err = cudaMemcpy(&best[i], &agent_val[ind[0]], sizeof(float), ::cudaMemcpyDeviceToHost);
	//}
	...
	for (int i = 0; i < max_iter; ++i) 
	{
		...
		calc_distances<<<agents,1>>>(agent_pos, X_gwo, dist_gwo);

		iGWO_nh << <agents, num_of_dims>> > (r_w, r, a, b, dist_gwo, agent_pos, X_gwo, nh_pos, ind_to_choose);

		cost_func << <1, num_of_agents >> > (nh_pos, nh_val);
		cost_func << <1, num_of_agents >> > (X_gwo, agent_val);

		cudaDeviceSynchronize();
		iGWO_compare_two_pop << <num_of_agents, num_of_dims >> > (agent_pos, agent_val, X_gwo, agent_val, nh_pos, nh_val);
	}
