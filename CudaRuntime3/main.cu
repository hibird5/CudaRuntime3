...
int main()
{
	...
	for (int i = 0; i < max_iter; ++i)
	{
		...
		cost_func << <num_of_agents, dims_to_log_half>> > (y_DE, y_DE_val);

		err = cudaMemcpy(pop_back, agent_pos, num_of_agents * sizeof(float), ::cudaMemcpyDeviceToHost);
		err = cudaMemcpy(pop_vals, agent_val, num_of_indices * sizeof(float), ::cudaMemcpyDeviceToHost);
		for (int i = 0; i < num_of_agents; ++i)
		{
			cout << i << "   " << pop_vals[i] << "----- " ;
			for (int j = 0; j < num_of_dims; ++j)
