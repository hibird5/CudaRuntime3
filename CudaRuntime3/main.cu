...
int main()
{
	...
	// prog
	get_constr << <num_of_dims, 1 >> > (-5, 5, a, b);
	...
	*/
	...
	dim3 agents(num_of_agents, num_of_agents);
	float* r;
	curandGenerator_t r_int;
	...
	for (int i = 0; i < max_iter; ++i)
	{
		curandGenerateNormal(r_int, r, num_of_uR, 0.0, 0.5);
		ffa << <agents, num_of_dims>> > (1, 1, 0.01, a, b, r, agent_pos, ff_new_poss, agent_val);
		cudaDeviceSynchronize();
		cost_func << <1, agents>> > (ff_new_poss, ff_new_vals);
		cudaDeviceSynchronize();
		//aaaaaaaaaaaaaaaaaaaaaaaaaaaa
		searchForBestKernel << <num_of_agents_half, num_of_agents>> > (agent_best_pso_v, indice);
